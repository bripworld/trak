...
public class ScanActivity extends Activity implements BeaconConsumer,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ronText = (EditText) ScanActivity.this.findViewById(R.id.ron);
        hermioneText = (EditText) ScanActivity.this.findViewById(R.id.hermione);
        dracoText = (EditText) ScanActivity.this.findViewById(R.id.draco);
    ...
    private void logBeaconData(Beacon beacon) {
        ...
        if (index) {
            scanString.append("sender=Bus101A&id=" + eventNum++);
        }
        ...
        if (!currentLocation.contentEquals(lastKnownLocation)) {
            StringBuilder locationBuilder = new StringBuilder();
            locationBuilder.append("loc=").append(currentLocation);
            locationBuilder.append("&t=").append(BeaconHelper.getCurrentTimeStamp());
            sendDataToServer(locationBuilder.toString(), false);
            lastKnownLocation = currentLocation;
        }

        sendDataToServer(scanString.toString(), true);
    ...
    private void logGenericBeacon(StringBuilder scanString, Beacon beacon) {
        ...
        if (proximity) {
            scanString.append("&prox=").append(BeaconHelper.getProximityString(beacon.getDistance()));
        }
        ...
        if (timestamp) {
            scanString.append("&t=").append(BeaconHelper.getCurrentTimeStamp());
        }
    }
    ...
    private void populateBeacon(Beacon beacon) {
        try {
            ...
            if (builder.length() > 0) {
                builder.append("&t=" + BeaconHelper.getCurrentTimeStamp());
    ...
    private void sendDataToServer(String content, boolean hasBeaconData) {
        try {
            if (content.length() == 0) {
                return;
            }
            if (hasBeaconData && !content.toLowerCase().contains("immediate")) {
                return;
            }

            String site = "http://smartbus-demo.ap-southeast-1.elasticbeanstalk.com/";
            String method = hasBeaconData ? "beacontrack.php?" : "bustrack.php?";
            String URL = site + method + content;
